...
class IA:
    def __init__(
        ...
        LLAMA_EMBEDDINGS_MODEL = f"{Path.home()}/.config/codeexplain/model.bin"
        MODEL_N_CTX = 1000
        ...
        PROMPT = PromptTemplate(
            template=PROMPT_TEMPLATE, input_variables=["language", "code"]
        )
        LLM = LlamaCpp(
            model_path=LLAMA_EMBEDDINGS_MODEL, n_ctx=MODEL_N_CTX, verbose=False
        )
...
class CodeExplain(object):
    ...
    def codeExplain(self, args, range):
        ...
        explained = self.codeExplainAI.run(
            {"language": programming_language, "code": selected_text}
        )
